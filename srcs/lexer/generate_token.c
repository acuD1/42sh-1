...
int		check_last_state(t_lexer *machine)
{
	int		i;

	i = 0;
	if (machine->last_state == E_STRING)
		return (check_script(machine));
	while (i < TOKEN_WITH_DATA)
	{
		if (machine->last_state == machine->duplicate[i])
	...
	{
		if (machine->last_state == machine->special_signs[i])
			return (machine->special_signs[i]);
		i++;
	}
	if (machine->last_state == E_IO_NUMBER)
		return (E_IO_NUMBER);
	if (machine->last_state == E_NEWLINE)
		return (E_NEWLINE);
	return (machine->last_state == E_END ? E_END : -1);
}
...
int		define_type(t_lexer *machine)
{
	...
	if ((result = check_last_state(machine)) >= 0)
		return (result);
	else if ((result = check_char(machine)))
		return (result);
	else if (machine->last_state == E_DB_QUOTE || machine->quote == QUOTE_ON)
		return (E_DB_QUOTE);
	return (E_STRING);
}
